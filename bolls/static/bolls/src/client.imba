...
tag App
  ...
  def addToChoosen id
    if choosen.find(do |element| return element == id)
      ...
      # console.log choosen_text.splice(choosen.indexOf(id), 1), choosen_text
    else
      choosen.push id
      # getChoosenVerse id
  ...
  def render
    <self>
      ...
      <div.settings-menu>
        ...
        <input[search:search_input].search_btn type='search' placeholder=lengdata:search :keydown.enter.getSearchText :keydown.esc=(do search:search_div = false)> "Search"
        ...
        <div.search_body id="search_body" tabindex="0">
          if search_verses:length
            if search:show_filters
              <div.filters>
                if parallel_text:edited_version == parallel_text:translation && parallel_text:display
                  <div.book_in_list :tap.prevent.dropFilter> lengdata:drop_filter
                  for book in @parallel_books
            ...
            if search:is_filter
              for verse, key in search_verses when verse:fields:book == search:filter
                ...
                if key > search:counter
                  <button.more_results :tap.prevent=(do search:counter = key + 50)> lengdata:more_results
                  break
              <div css:padding='12px 0px'>
                lengdata:filter_name, ' ', nameOfBook search:filter
              ...
              for verse, key in search_verses
                <a.search_item href="/{verse:fields:translation}/{verse:fields:book}/{verse:fields:chapter}/#{verse:fields:verse}" target="_blank">
                  ...
                  <div.search_res_verse_header>
                    ...
                    <span> verse:fields:verse
                if key > search:counter
                  <button.more_results :tap.prevent=(do search:counter += 50)> lengdata:more_results
                  break
          else
            <p> lengdata:nothing
